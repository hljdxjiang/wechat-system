...
public class UserOperaRecordController {
    ...
    public MapResponse queryByPage(@RequestBody UserOperaRecordEntity userOperaRecordEntity, @RequestBody PageQueryBaseDTO dto) {
        ...
        // userOperaRecordEntity.setTenantId(SessionKey.getTenantId);
    ...
    public MapResponse fuzzyQuery(@RequestBody UserOperaRecordEntity userOperaRecordEntity, @RequestBody PageQueryBaseDTO dto) {
        MapResponse mapResponse = new MapResponse();
        logger.info("fuzzyQuery begin.userOperaRecordEntity:{},dto:{}", JsonUtil.obj2Json(userOperaRecordEntity),JsonUtil.obj2Json(dto));
        PageHelper.startPage(dto.getPageNum(), dto.getPageSize());
        // userOperaRecordEntity.setTenantId(SessionKey.getTenantId);
        List<UserOperaRecordEntity> list = this.userOperaRecordService.fuzzyQuery(userOperaRecordEntity);
        PageInfo pageInfo = new PageInfo(list);
        Map map = PageInfoUtil.parseReturnMap(pageInfo);
        mapResponse.setData(map);
        logger.info("queryByPage end.mapResponse:{}", JsonUtil.obj2Json(mapResponse));
     ...
     */
    @PostMapping("/add")
    public MapResponse add(@RequestBody UserOperaRecordEntity userOperaRecordEntity) {
     ...
     */
    @PostMapping("/edit")
    public MapResponse edit(@RequestBody UserOperaRecordEntity userOperaRecordEntity) {
        MapResponse mapResponse = new MapResponse();
     ...
     */
    @PostMapping("/delete")
    public MapResponse deleteById(@RequestBody UserOperaRecordEntity userOperaRecordEntity) {
        ...
        this.userOperaRecordService.deleteById(id);
