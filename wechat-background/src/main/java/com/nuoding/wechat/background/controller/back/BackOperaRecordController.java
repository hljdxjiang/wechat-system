...
public class BackOperaRecordController {
    ...
    public MapResponse queryByPage(@RequestBody BackOperaRecordEntity backOperaRecordEntity, @RequestBody PageQueryBaseDTO dto) {
        ...
        logger.info("queryByPage begin.backOperaRecordEntity:{},dto:{}", JsonUtil.obj2Json(backOperaRecordEntity),JsonUtil.obj2Json(dto));
        PageHelper.startPage(dto.getPageNum(), dto.getPageSize());
        // backOperaRecordEntity.setTenantId(SessionKey.getTenantId);
    ...
    public MapResponse fuzzyQuery(@RequestBody BackOperaRecordEntity backOperaRecordEntity, @RequestBody PageQueryBaseDTO dto) {
        MapResponse mapResponse = new MapResponse();
        logger.info("fuzzyQuery begin.backOperaRecordEntity:{},dto:{}", JsonUtil.obj2Json(backOperaRecordEntity),JsonUtil.obj2Json(dto));
        PageHelper.startPage(dto.getPageNum(), dto.getPageSize());
        // backOperaRecordEntity.setTenantId(SessionKey.getTenantId);
        List<BackOperaRecordEntity> list = this.backOperaRecordService.fuzzyQuery(backOperaRecordEntity);
        PageInfo pageInfo = new PageInfo(list);
        Map map = PageInfoUtil.parseReturnMap(pageInfo);
        mapResponse.setData(map);
        logger.info("queryByPage end.mapResponse:{}", JsonUtil.obj2Json(mapResponse));
     ...
     */
    @PostMapping("/add")
    public MapResponse add(@RequestBody BackOperaRecordEntity backOperaRecordEntity) {
        logger.info("add begin.backOperaRecordEntity:{}",JsonUtil.obj2Json(backOperaRecordEntity));
        MapResponse mapResponse = new MapResponse();
        // backOperaRecordEntity.setTenantId(SessionKey.getTenantId);
     ...
     */
    @PostMapping("/edit")
    public MapResponse edit(@RequestBody BackOperaRecordEntity backOperaRecordEntity) {
        MapResponse mapResponse = new MapResponse();
        logger.info("edit begin.backOperaRecordEntity:{}",JsonUtil.obj2Json(backOperaRecordEntity));
     ...
     */
    @PostMapping("/delete")
    public MapResponse deleteById(@RequestBody BackOperaRecordEntity backOperaRecordEntity) {

        MapResponse mapResponse = new MapResponse();
        logger.info("deleteById begin.backOperaRecordEntity:{}",JsonUtil.obj2Json(backOperaRecordEntity));
        Integer id = backOperaRecordEntity.getId();
        if (id == null || id == 0) {
            mapResponse.setResponse(RespStatusEnum.ARGS_ERROR);
            return mapResponse;
        }
        this.backOperaRecordService.deleteById(id);
