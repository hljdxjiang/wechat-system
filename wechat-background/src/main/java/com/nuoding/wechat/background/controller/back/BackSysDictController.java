...
public class BackSysDictController {
    ...
    public MapResponse queryByPage(@RequestBody BackSysDictEntity backSysDictEntity, @RequestBody PageQueryBaseDTO dto) {
        ...
        // backSysDictEntity.setTenantId(SessionKey.getTenantId);
    ...
    public MapResponse fuzzyQuery(@RequestBody BackSysDictEntity backSysDictEntity, @RequestBody PageQueryBaseDTO dto) {
        MapResponse mapResponse = new MapResponse();
        logger.info("fuzzyQuery begin.backSysDictEntity:{},dto:{}", JsonUtil.obj2Json(backSysDictEntity), JsonUtil.obj2Json(dto));
        PageHelper.startPage(dto.getPageNum(), dto.getPageSize());
        // backSysDictEntity.setTenantId(SessionKey.getTenantId);
        List<BackSysDictEntity> list = this.backSysDictService.fuzzyQuery(backSysDictEntity);
        PageInfo pageInfo = new PageInfo(list);
        Map map = PageInfoUtil.parseReturnMap(pageInfo);
        mapResponse.setData(map);
        logger.info("queryByPage end.mapResponse:{}", JsonUtil.obj2Json(mapResponse));
     ...
     */
    @PostMapping("/add")
    public MapResponse add(@RequestBody BackSysDictEntity backSysDictEntity) {
     ...
     */
    @PostMapping("/edit")
    public MapResponse edit(@RequestBody BackSysDictEntity backSysDictEntity) {
        MapResponse mapResponse = new MapResponse();
     ...
     */
    @PostMapping("/delete")
    public MapResponse deleteById(@RequestBody BackSysDictEntity backSysDictEntity) {
        ...
        this.backSysDictService.deleteById(id);
