...
service="wecaht-background"
jar_name="wecaht-background-1.0.0-SNAPSHOT.jar"
...
EXEC_LOG_DIR="/home/wechat/logs/wechat_background"
...
if [[ -z "$PIDS" ]]; then
	echo "[INFO] The $service service already stoped!"
	echo "[INFO] PID: $PIDS"
	exit 0
fi
...
sleep 2
COUNT=`ps ux | grep "$service" | grep "$server_port" | awk '{print $2}' | wc -l`
if [ $COUNT -gt 0 ]; then
